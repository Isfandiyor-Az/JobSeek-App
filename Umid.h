...
fstream fin("job.dat", ios::binary | ios::in | ios::out);
    ...
    static void DeleteJob()
    {
        ...
        remove("job.dat");
        rename("temp.dat", "job.dat");
        cout<<"Job successfully deleted!"<<endl;
    }
static void ApplicationSystem(Employee &employee_obj)
{
    fstream app_file("application.dat", ios::binary | ios::in | ios::out);
...
ifstream job_file("job.dat", ios::binary); // Corrected to read from job.dat
    Job job_obj;
    bool find = false;
    while (job_file.read(reinterpret_cast<char*>(&job_obj), sizeof(job_obj)))
    {
        if (strcmp(search_name, job_obj.getJobName()) == 0)
        {
            ...
            if (choice == 'y' || choice == 'Y')
            {
                ...
                // Write to application file
                app_file.clear();
                app_file.seekp(0, ios::end); // Append to the end of the file
                app_file.write(reinterpret_cast<char*>(&search_name), sizeof(search_name));
                int temp_id = employee_obj.get_ID();
                app_file.write(reinterpret_cast<char*>(&temp_id), sizeof(temp_id));
                // Update job file (optional, depending on your design)
                // For now, we'll assume job.dat is updated separately
            }
            else
            {
                cout << "You have not applied for this job.\n";
    ...
    static void ReadFromApplicationFile(Employee &obj)
{
    int currentEmployeeID = obj.get_ID();
    ifstream app_file("application.dat", ios::binary);
    if (!app_file)
    {
        cerr << "Error opening application file. No jobs applied yet.\n";
        return;
    }
    ...
    while (app_file.read(reinterpret_cast<char*>(&job_name), sizeof(job_name)))
    {
        if (app_file.read(reinterpret_cast<char*>(&employee_id), sizeof(employee_id)))
        {
            if (employee_id == currentEmployeeID)
            {
                found = true;
                Job appliedJob = Job::R_searchJobByKeyword(job_name);
                if (strcmp(appliedJob.getJobName(), "Unknown") != 0) // Check if job is found
                {
                    cout << "\n\t\t\t --- Job " << jobCount++ << " ---\n";
                    appliedJob.DisplayJobInfo();
                }
                else
                {
                    cout << "Job with name '" << job_name << "' not found in job.dat.\n";
                }
            }
        }
        else
    ...
    if (!found)
    {
        cout << "No jobs applied by this employee.\n";
    }
